...
c---------------------------------------------------------------------
     ...
     >                   epsilon, xce(5), xcr(5), dtref
        integer m, no_time_steps, istat
        character class
        logical verified
        integer iSize, jSize, kSize, blockSize, sharedSize
        integer i,j,k
...
c---------------------------------------------------------------------
        ...
        istat = cudaMemcpy(u_r, u_dv_r, 5*(IMAXP+1)*(JMAXP+1)*KMAX)
...
c---------------------------------------------------------------------
        ...
        istat= cudaThreadSynchronize()
        if (timeron) call timer_start(t_gcomm)
        istat = cudaMemcpy(rhs_r, rhs_dv_r,5*(IMAXP+1)*(JMAXP+1)*KMAX)
        if (timeron) call timer_stop(t_gcomm)
...
c---------------------------------------------------------------------
        ...
        if (class .ne. 'U') then
           write(*, 3990) class
 3990      format(' Verification being performed for class ', a)
           write (*,4000) epsilon
 4000      format(' accuracy setting for epsilon = ', E20.13)
           verified = (dabs(dt-dtref) .le. epsilon)
           if (.not.verified) then  
              class = 'U'
              write (*,3000) dtref
 3000         format(' DT does not match the reference value of ', 
     >                 E15.8)
           endif
        else 
           write(*, 3995)
 3995      format(' Unknown class')
        endif
 ...
 4001   format(' Comparison of RMS-norms of residual')
 4005   format(' RMS-norms of residual')
        do m = 1, 5
           if (class .eq. 'U') then
              write(*, 4015) m, xcr(m)
           else if (xcrdif(m) .le. epsilon) then
              write (*,4011) m,xcr(m),xcrref(m),xcrdif(m)
           else 
              verified = .false.
              write (*,4010) m,xcr(m),xcrref(m),xcrdif(m)
           endif
        enddo
 ...
 4002   format(' Comparison of RMS-norms of solution error')
 4006   format(' RMS-norms of solution error')
        
        do m = 1, 5
           if (class .eq. 'U') then
              write(*, 4015) m, xce(m)
           else if (xcedif(m) .le. epsilon) then
              write (*,4011) m,xce(m),xceref(m),xcedif(m)
           else
              verified = .false.
              write (*,4010) m,xce(m),xceref(m),xcedif(m)
           endif
        enddo
        
 4010   format(' FAILURE: ', i2, E20.13, E20.13, E20.13)
 4011   format('          ', i2, E20.13, E20.13, E20.13)
 4015   format('          ', i2, E20.13)
        
        if (class .eq. 'U') then
           write(*, 4022)
           write(*, 4023)
 4022      format(' No reference values provided')
 4023      format(' No verification performed')
        else if (verified) then
           write(*, 4020)
 4020      format(' Verification Successful')
        else
           write(*, 4021)
 4021      format(' Verification failed')
        endif
