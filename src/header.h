...
c---------------------------------------------------------------------
      ...
      integer           grid_points(3), nx2, ny2, nz2, doGPU, doWrite,
     >                  sizeOfDouble,sizeOfInt,dfltSubSections,
     >                  syncForTimers,fixedBlockSize,fixedBlockWidth, 
     >                  fixedBlockHeight
      logical           timeron
      common /global/   grid_points, nx2, ny2, nz2, timeron, doGPU,
     >                  doWrite, sizeOfDouble, sizeOfInt, 
     >                  dfltSubSections, syncForTimers, fixedBlockSize,
     >                  fixedBlockWidth, fixedBlockHeight
...
c-----------------------------------------------------------------------
     ...
     >        t_rhs,t_txinvr,t_last,t_total, t_temp, t_kernel,
     >        t_gcomm
      parameter (t_total = 1)
      parameter (t_rhsx = 2)
      parameter (t_rhsy = 3)
      ...
      parameter (t_add = 15)
      parameter (t_temp = 16)
      parameter (t_kernel = 17)
      parameter (t_gcomm = 18)
      parameter (t_last = 18)
